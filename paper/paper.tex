...
\author{Rodion Orkin\\ Independent Researcher\\ \texttt{rodion.ork@yandex.ru}}
\date{}
...
recovery is $40/40$, $40/40$ and $38/40$ at depths 3, 4, 5 at the
default budget — 0.00\,s, 0.03\,s and 28\,s per target — and
$\mathbf{40/40}$ at depth 5 including the final escalation rung
(unranked streamed verification of every generated candidate pair; the
two hardest targets fall after 12M and 42M verified pairs) — versus at
most $1/8$ for
...
The machinery is operator-generic: repeated verbatim for
$\psi=\sinh-\operatorname{arsinh}$ — the harder enumeration case,
whose unclamped pool is $2.3\times$ larger — verified recovery is
$20/20$ at every depth 3--5.
...
depth-critical targets, and an independent follow-up study finds
\emph{balanced} tree shapes unrecoverable by gradient training even
at depth 3 — $0/3\,776$ trials across three architectures and three
operators~\cite{gupta2026arch}. Because the snap space grows doubly
...
5 & $8.8\times10^{23}$ & \textbf{38/40} \,(28 s)   & 0/8 \,(2 MSE-hits) & 1/8 \\
\bottomrule
\end{tabular}
...
contradiction); it dies at depth 5. The two default-budget depth-5 misses are fully
characterised (their correct pairs rank deep inside clamp-degenerate
match classes) and both fall to the ladder's last rung — unranked
streamed verification of every generated pair, no caps — after 11.7M
(164 s) and 42.3M (557 s) verified candidates: depth-5 recovery is
40/40 with the full ladder, and no unexplained failure remains at any
depth.}
\label{tab:recovery}
\end{table}

Wilson 95\% intervals for the headline proportions: $40/40 \to
[91.2\%, 100\%]$ and $38/40 \to [83.5\%, 98.6\%]$; the gap to the
baselines (at most $1/8$) is far outside any overlap. The escalation
ladder — ranked default, extended caps with multi-resolution
matching, then unranked streamed verification — completes depth-5
recovery to $40/40$, with each rung's budget and outcome recorded in
\texttt{depth\_wall\_results.json}.

\paragraph{Operator generality.} The pool machinery touches the
operator only through node evaluation and the two side
canonicalisations, so the identical pipeline runs verbatim on
$\psi(x,y)=\sinh(x)-\operatorname{arsinh}(y)$ — the \emph{harder}
enumeration case: with no clamps there is no functional collapse
(level-3 pool $=$ all $1\,444$ live trees; level-4 pool
$1\,431\,542$, $2.3\times$ EML's), and root inversion is exact
($a=\operatorname{arsinh}(y+\operatorname{arsinh} b)$).
Depth-critical verified recovery is $\mathbf{20/20}$ at each of
depths 3, 4, 5 (0.00\,s, 0.01\,s, 39.5\,s per target), and zero
random $\psi$ targets collapse to shallower functions — no
degeneracy means every sampled tree is genuinely depth-critical
(\texttt{psi\_depth\_wall\_results.json}).
...
\paragraph{Single-operator representations.} Universal-operator
reductions trace through logarithm tables, the exp--log
representation, and Euler's formula; the single-binary-operator
endpoint is Odrzywo{\l}ek's discovery~\cite{odrzywolek2026eml}, which
also frames the trainable master-formula architecture and reports the
depth wall we study.

\paragraph{Symbolic regression.} Modern SR searches syntax
stochastically: genetic programming (Eureqa~\cite{schmidt2009},
PySR~\cite{cranmer2023pysr}), risk-seeking policy gradients
(DSR~\cite{petersen2021dsr}), continuous relaxation via
Gumbel-softmax~\cite{jang2017gumbel}, physics-inspired decomposition
(AI Feynman~\cite{udrescu2020feynman}), and pretrained transformers
(NeSymReS~\cite{biggio2021}, end-to-end
SR~\cite{kamienny2022}). SRBench~\cite{lacava2021} standardises the
empirical comparison, and SR over unrestricted grammars is NP-hard in
general~\cite{virgolin2022}. Our results carve out a regime where
search is unnecessary: for single-operator grammars under the clamped
semantics that any trainable implementation must adopt, the reachable
function space is small enough to enumerate outright, which subsumes
stochastic search below the enumeration horizon and reduces it to
residual inversion above.

\paragraph{Exhaustive and enumerative approaches.} Bottom-up
enumeration with observational-equivalence pruning is a core
technique in program synthesis — TRANSIT~\cite{udupa2013}, recursive
synthesis with equivalence reduction~\cite{albarghouthi2013}, and
the enumerative solvers of the SyGuS competition~\cite{alur2013}.
Within SR itself, Prioritized Grammar Enumeration~\cite{worm2013},
exhaustive search with semantic structure
deduplication~\cite{kammerer2021}, and Exhaustive Symbolic
Regression~\cite{bartlett2023esr} enumerate all expressions of a
multi-operator basis up to ${\sim}10$ tree nodes, deduplicate
syntactically/semantically, and fit each candidate's parameters
numerically. Our contribution is not the enumeration schema but (i) the \emph{measurement} that a
trainable SR architecture's function space collapses by five-plus
orders of magnitude under its own numerical semantics, (ii) the
identification of that collapse with the optimiser-killing clamp
saturation (the wall's cause and cure coincide), (iii) the
$O(\text{pool})$ analytic \emph{root inversion} — a meet-in-the-middle
step in the spirit of cryptanalytic time--memory
trade-offs~\cite{diffie1977} — with informativeness-ranked matching,
and (iv) verification-gated recovery with certificates on failure as
well as success. Structurally, the single-operator grammar plus
value-vector dedup under the trained clamped semantics plus root
inversion reaches expression sizes (up to 63 nodes at depth 5) far
beyond the ${\sim}10$-node ceilings of syntactic-dedup exhaustive
SR, at the cost of grammar specificity.

\paragraph{EML follow-up literature.} The three months since the
source paper have produced universality results for EML trees as
approximators, algebraic analyses of the operator, hardware
instantiations, and a computability-theoretic inexpressibility
theorem (EML numbers coincide with Chow's EL numbers and exclude
non-computable reals such as Chaitin's
$\Omega$)~\cite{carney2026}. On the training side, the follow-up
closest to our question documents the wall in sharper form —
architecture--target interactions and universally unrecoverable
balanced shapes~\cite{gupta2026arch} — while remaining inside the
optimisation paradigm. To our knowledge no prior work enumerates the
EML function space, inverts the root analytically, or reports
verified exact recovery beyond depth 4.

\paragraph{Deployable additive models.} GA$^2$M/EBM~\cite{lou2013accurate}
provides the additive scaffold and the flat-beyond-range behaviour
that SafePoolGAM adopts as its fallback; SafePoolGAM replaces spline
shapes with closed forms from a complete atlas and adds
tail-certified extrapolation gates, which no tree-based additive
model can offer.
...
The author initiated and directed this research programme, set its
questions and quality bar, and reviewed and validated the results,
and takes full responsibility for all claims. A large language model
agent (Anthropic Claude) was used substantially throughout the work
— in algorithm design, implementation, experimentation, and
manuscript drafting — under the author's direction. Every
quantitative result was produced by the released code under seeded
protocols and re-verified on independent grids
(\texttt{REPRODUCIBILITY.md}); none of the reported numbers
originates from model text generation. This disclosure follows the
practice of the source paper~\cite{odrzywolek2026eml}.
...
\emph{Interpretable machine learning for science with PySR and
SymbolicRegression.jl}. arXiv:2305.01582, 2023.
\bibitem{petersen2021dsr} B.~K.~Petersen, M.~Landajuela, T.~N.~Mundhenk,
C.~P.~Santiago, S.~Kim, J.~T.~Kim.
\emph{Deep symbolic regression: recovering mathematical expressions
...
\bibitem{ax1971} J.~Ax. \emph{On Schanuel's conjectures}. Annals of
Mathematics 93:252--268, 1971.
...
\bibitem{schmidt2009} M.~Schmidt, H.~Lipson.
\emph{Distilling free-form natural laws from experimental data}.
Science 324(5923):81--85, 2009.
\bibitem{biggio2021} L.~Biggio, T.~Bendinelli, A.~Neitz, A.~Lucchi,
G.~Parascandolo. \emph{Neural symbolic regression that scales}.
ICML 2021.
\bibitem{kamienny2022} P.-A.~Kamienny, S.~d'Ascoli, G.~Lample,
F.~Charton. \emph{End-to-end symbolic regression with transformers}.
NeurIPS 2022.
\bibitem{lacava2021} W.~La~Cava, P.~Orzechowski, B.~Burlacu,
F.~O.~de~Fran\c{c}a, M.~Virgolin, Y.~Jin, M.~Kommenda, J.~H.~Moore.
\emph{Contemporary symbolic regression methods and their relative
performance}. NeurIPS Datasets and Benchmarks 2021.
\bibitem{virgolin2022} M.~Virgolin, S.~P.~Pissis.
\emph{Symbolic regression is NP-hard}. TMLR, 2022.
\bibitem{udupa2013} A.~Udupa, A.~Raghavan, J.~V.~Deshmukh,
S.~Mador-Haim, M.~M.~K.~Martin, R.~Alur.
\emph{TRANSIT: specifying protocols with concolic snippets}. PLDI 2013.
\bibitem{albarghouthi2013} A.~Albarghouthi, S.~Gulwani, Z.~Kincaid.
\emph{Recursive program synthesis}. CAV 2013.
\bibitem{alur2013} R.~Alur, R.~Bod\'ik, G.~Juniwal, M.~M.~K.~Martin,
M.~Raghothaman, S.~A.~Seshia, R.~Singh, A.~Solar-Lezama, E.~Torlak,
A.~Udupa. \emph{Syntax-guided synthesis}. FMCAD 2013.
\bibitem{diffie1977} W.~Diffie, M.~E.~Hellman.
\emph{Exhaustive cryptanalysis of the NBS Data Encryption Standard}.
Computer 10(6):74--84, 1977.
\bibitem{bartlett2023esr} D.~J.~Bartlett, H.~Desmond, P.~G.~Ferreira.
\emph{Exhaustive symbolic regression}. IEEE Transactions on
Evolutionary Computation 28(4), 2024 (arXiv:2211.11461).
\bibitem{kammerer2021} L.~Kammerer, G.~Kronberger, B.~Burlacu,
S.~M.~Winkler, M.~Kommenda, M.~Affenzeller. \emph{Symbolic
regression by exhaustive search: reducing the search space using
syntactical constraints and efficient semantic structure
deduplication}. In Genetic Programming Theory and Practice XVIII,
2021 (arXiv:2109.13895).
\bibitem{worm2013} T.~Worm, K.~Chiu. \emph{Prioritized grammar
enumeration: symbolic regression by dynamic programming}. GECCO 2013.
\bibitem{gupta2026arch} C.~Gupta. \emph{Why architecture choice
matters in symbolic regression}. arXiv:2604.23256, 2026.
\bibitem{carney2026} M.~Carney. \emph{Inexpressibility in
Exp-Minus-Log}. arXiv:2605.01636, 2026.
\end{thebibliography}
