...
recovery is $40/40$, $40/40$ and $38/40$ at depths 3, 4, 5 versus
$0$--$8\%$ for multi-start Adam and ${\sim}0\%$ for uniform syntactic
sampling; the source paper's own bivariate showcase
...
worse), and it never finishes catastrophically last. The earlier
...
with temperature annealing (10 restarts $\times$ 1200 epochs, the
reference approach), and uniform syntactic snap sampling with OLS
readout (100k samples).

\begin{table}[t]
\centering\small
\begin{tabular}{lcccc}
\toprule
depth & syntactic snaps & pool (this work) & gradient descent & uniform 100k \\
\midrule
3 & $1.9\times10^{5}$  & \textbf{40/40} \,(0.0 s) & TBD & TBD \\
4 & $3.1\times10^{11}$ & \textbf{TBD} & TBD & TBD \\
5 & $8.8\times10^{23}$ & \textbf{TBD} & TBD & TBD \\
\bottomrule
\end{tabular}
\caption{Verified exact recovery on depth-critical random targets
(40 targets per depth; GD and uniform on 12-target subsets for
compute parity). Mean seconds per target in parentheses.}
\label{tab:recovery}
\end{table}
...
target depth & 6 & 7 & 8 \\
\midrule
recovered by depth-5 solver & TBD & TBD & TBD \\
\bottomrule
\end{tabular}
...
extrapolation frontier, gated per feature (free versus clipped
extension, decided on the tails), and wrapped in a declared output
envelope. Catastrophic extrapolation is excluded by construction;
genuine closed-form extrapolation survives the gates.

On the nine-dataset UCI physical-extrapolation suite of the earlier
project iteration (train below a physical threshold, test above),
SafePoolGAM keeps the yacht win with the recovered law
$0.076\,e^{13.3\,\mathrm{Fr}}$ ($R^2=+0.63$, versus $-0.8$/$-1.1$ for
linear/tree baselines) and — unlike the unguarded EML-GA$^2$M, which
fails catastrophically on six of nine datasets (down to
$R^2=-2.7\times10^{7}$ in the re-run) — never finishes with a
catastrophic score. Full per-dataset numbers: Table~\ref{tab:uci}.
...
\midrule
\multicolumn{6}{c}{TBD — filled from \texttt{safe\_uci\_results.json}} \\
\bottomrule
\end{tabular}
